...
- Não contrarie o que está em vibing/docs/adr/. Se precisar contrariar,
  pare.
...
- Não gere scaffold automático de framework sem mostrar antes o que ele
  vai criar.
- Não escreva código de funcionalidade sem uma spec APROVADA em
  vibing/docs/specs/. Você pode rascunhar a spec; rascunho não é
  aprovação.
...
- O único projeto Supabase é produção. Não existe outro banco.
- Não altere o Supabase fora de uma migration: nada de SQL avulso, de
  Studio, de alteração de schema ou de dado. Migration só é aplicada
  depois que eu aprovar o SQL (vibing/rules/migration.md).
- Não rode comando que apague dado ou resete o banco.
- Não faça push na main, que é produção. Trabalhe em branch, faça push
  da branch e abra PR.
- Não faça merge. O merge é meu.
