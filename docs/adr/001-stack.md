...
- **Projetos na Vercel** — justificativa não ditada.
...
- **Camadas** — não existe View no servidor, o front é uma SPA separada; o Repository isola o Prisma para o service não depender de ORM.
...
- **Fonte do contrato** — o OpenAPI atravessa a fronteira como arquivo; ts-rest exigiria que os dois lados importassem o mesmo pacote, que não existe com repositórios separados.
- **Distribuição** — justificativa não ditada.
...
- **Objetos não cobertos pelo Prisma** — RLS, policies e triggers não são modelados pelo Prisma, então precisam entrar na mesma linha do tempo versionada, não como script solto aplicado à mão.
- **Conexão de runtime** — `connection_limit=1` é obrigatório em serverless, não recomendação: a Vercel escala instâncias em paralelo e cada uma abrindo pool próprio esgota a conexão do projeto.
...
- **Sessão no navegador** — `vercel.app` está na Public Suffix List, então o navegador proíbe cookie no domínio pai e o cookie `httpOnly` não atravessaria entre front e API em subdomínios diferentes; com o `supabase-js`, renovação de token e persistência de sessão já vêm resolvidas, e escrever isso à mão adicionaria um interceptor com fila de refresh sem ganho de segurança, já que o token ficaria acessível ao JavaScript de qualquer forma.
- **Domínio** — justificativa não ditada.
...
- **CORS** — justificativa não ditada.
- **Fonte da verdade de autorização** — as regras são por papel e por tenant ao mesmo tempo; `if (user.role === 'admin')` espalhado por controller não sobrevive à terceira regra.
- **RLS** — se uma credencial vazar ou um endpoint esquecer o guard, a policy é a última barreira entre tenants.
- **Papel do RLS** — o Prisma conecta com um role dono das tabelas, que bypassa RLS por padrão; com um Nest no meio, as policies só rodariam se cada request abrisse transação com `SET LOCAL role` e `SET LOCAL request.jwt.claims`, o que custa uma transação por request e briga com o pooler.
...
- **Meta de cobertura** — meta de percentual produz teste escrito para subir número; caminho crítico é critério verificável.
...
- **Região das functions** — São Paulo é oferecida apenas no Pro; no Hobby as functions rodam nos EUA e cada query paga ida e volta transatlântica, o que é aceitável em contexto de demonstração e é o primeiro item a mudar se o projeto for usado de verdade.
- **Região do Supabase** — justificativa não ditada.
...
- **Fila e trabalho assíncrono** — não há volume assíncrono conhecido agora; adicionar infraestrutura de fila sem caso de uso é custo sem retorno, e a consequência é que envio de e-mail e integração externa rodam dentro do request e seguram a resposta.
- **Agendamento** — sem caso de uso agendado no escopo atual.
...
- **Segredos** — justificativa não ditada.
